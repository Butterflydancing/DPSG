...
def embed_text(ids, texts, max_seq_len, config, dir_name, num_process = 8):
    def skip_existed(id_list, text_list, dir_name):
        ...
        existed = set([fn for fn in os.listdir(dn) if os.stat(os.path.join(dn,fn)).st_size > 0]) if not os.path.isdir(dn) else set()#返回一个包含指定目录中所有非空文件名的集合，如果目录不存在或是一个目录（而非文件），则返回一个空集合
